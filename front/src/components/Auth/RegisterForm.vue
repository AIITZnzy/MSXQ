...
const goAnotherForm=()=>{
  emits("update:isLoginComponent",true)
}
...
const loginHandler=()=>{
  post('api/user/login',{
    ...
    messageApi.success(message+'注册成功')
    setTimeout(()=>{
      // ....................记得补充
    },3000)
  },(message,data)=>{
      ...
      <div class="c">
        <div class="d text-center">
          ...
          <div class="link-container">
            <div @click="goAnotherForm" class="text-blue-400 hover:text-blue-500 hover:underline cursor-pointer">马上登入</div>
            <div>已有账号</div>
          </div>
          <button @click="loginHandler()" class="g">注册</button>
        </div>
      </div>
    </div>
  </div>
